...
function adminGetContactQueries() {
  const queries = getContactQueries();
  return queries.slice().sort((a, b) => {
    const aTs = new Date(a.lastUpdatedAt || a.createdAt || 0).getTime();
    const bTs = new Date(b.lastUpdatedAt || b.createdAt || 0).getTime();
    return bTs - aTs;
  });
}

function adminReplyToContactQuery(queryId, message) {
  const session = getAdminSession();
  const senderName = session?.name || 'Admin Support';
  const result = addDeveloperReplyToContactQuery(queryId, message, {
    senderType: 'admin',
    senderName
  });
  return result;
...
function adminSetContactQueryStatus(queryId, status) {
  return updateContactQueryStatus(queryId, status);
}

function adminDeleteContactQuery(queryId) {
